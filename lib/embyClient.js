...
// --- Silo Item Finding ---
// Silo exposes ProviderIds through its Jellyfin compatibility API, but does not
// currently support Emby's ImdbId/TmdbId/TvdbId/AnyProviderIdEquals filters.
// For Silo only, keep a short-lived local provider-ID index. The Emby path below
// remains unchanged.
...
function addSiloIndexItem(index, item) {
    ...
    const bucket = type === 'movie' ? index.movie : type === 'series' ? index.series : null;
    if (!bucket) return;
...
async function buildSiloProviderIndex(config, force = false) {
    ...
    while (startIndex < total) {
        const data = await makeApiRequest(`${config.serverUrl}/Users/${config.userId}/Items`, {
            ...
            Recursive: true,
            Fields: 'ProviderIds,Name,Id,MediaSources',
    ...
    console.log(`🧱 Silo provider index built (${startIndex} items scanned, ${index.movie.imdb.size} IMDb movies, ${index.series.imdb.size} IMDb series)`);
    return index;
}
...
async function findSiloItems(kind, imdbId, tmdbId, tvdbId, anidbId, config) {
    ...
    let found = findInSiloIndex(index, kind, imdbId, tmdbId, tvdbId, anidbId);

    // Refresh once on a miss so newly-added media can resolve immediately.
    if (found.length === 0) {
        index = await buildSiloProviderIndex(config, true);
        found = findInSiloIndex(index, kind, imdbId, tmdbId, tvdbId, anidbId);
    }
    return found;
}
    ...
    for (const source of playbackInfoData.MediaSources) {
        try {
          ...
          qualityTitle = qualityTitle || 'Direct Play'; // Fallback title
            ...
            // Construct direct play URL (Emby format; also supported by Silo's compatibility API)
