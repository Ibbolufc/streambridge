...
function chooseCandidate(items, metadata) {
    ...
    if (wantedYear) {
        const exactTitleAndYear = exactTitle.find(item => Number(item?.ProductionYear) === wantedYear);
        if (exactTitleAndYear) return exactTitleAndYear;
    }

    if (exactTitle.length === 1) return exactTitle[0];
    if (exactTitle.length > 1) {
        return exactTitle.find(item => Number(item?.ProductionYear) === wantedYear) || exactTitle[0];
    }
...
async function findSiloItem(kind, imdbId, tmdbId, tvdbId, anidbId, config) {
    ...
    const item = chooseCandidate(data?.Items || [], metadata);
    if (!item) {
        console.warn(`⚠️ Silo search found no confident ${kind} match for "${metadata.name}"`);
        return [];
    }

    cacheSet(resolutionCache, cacheKey, item);
    console.log(`✅ Silo matched ${kind}: "${item.Name}"${item.ProductionYear ? ` (${item.ProductionYear})` : ''}`);
    return [item];
}
    ...
    try {
        if (parsed.itemType === ITEM_TYPE_MOVIE) {
            ...
            for (const item of items) {
                const streams = await getPlaybackStreams(item, null, config);
