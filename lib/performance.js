...
//   2) caches complete getStream() results;
//   3) coalesces identical simultaneous lookups into one Emby resolution.
...
class TTLCache {
  ...
  get(key) {
    ...
    // Move hot entries to the end so pruning behaves like a small LRU cache.
    this.map.delete(key);
...
// Reuse connections to Emby instead of repeatedly opening new TCP/TLS sessions.
...
// Load the normal working resolver, then wrap only its public getStream method.
// index.js will receive this same cached module instance when it requires embyClient.
...
console.log('⚡ StreamBridge safe performance layer enabled');
